...
} ctests[] =
{
  ...
  { "$6$9mBt0ZV/F0ci35SE", "soccer22",
    "$6$9mBt0ZV/F0ci35SE$vA1CvjX2E363qJSKxkfx/ZuZJWr2RlVpk5LNYOi.fAYTNxD2gQ9tvJPgeNLyfazrfDS2iNdghXmUUpqHDxrqt1"}
};
...
void test()
{
  ...
  for (i = 0; i < nctests; ++i)
    {
      char *cp = __sha512_crypt(ctests[i].input, ctests[i].salt);

      if (strcmp (cp, ctests[i].expected) != 0)
	{
	  printf ("test %d: expected \"%s\", got \"%s\"\n",
		  i, ctests[i].expected, cp);
	  result = 1;
	}
      else
      {
